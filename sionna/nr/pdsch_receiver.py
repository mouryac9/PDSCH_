...
#
"""PDSCH Receiver for the nr (5G) sub-package of the Sionna library.
"""
import numpy as np
...
class PDSCHReceiver(Layer):
    # pylint: disable=line-too-long
    r"""PDSCHReceiver(PDSCH_transmitter, channel_estimator=None, mimo_detector=None, tb_decoder=None, return_tb_crc_status=False, stream_management=None, input_domain="freq", l_min=None, dtype=tf.complex64, **kwargs)

    This layer implements a full receiver for batches of 5G NR PDSCH slots sent
    ...
    .. figure:: ../figures/PDSCH_receiver_block_diagram.png
        :scale: 30%
        :align: center
    ...
    :class:`~sionna.nr.PDSCHLSChannelEstimator`. If ``channel_estimator``
    ...
    Parameters
    ----------
    PDSCH_transmitter : :class:`~sionna.nr.PDSCHTransmitter`
    ...
    channel_estimator : :class:`~sionna.ofdm.BaseChannelEstimator`, "perfect", or `None`
        ...
        If `None`, the :class:`~sionna.nr.PDSCHLSChannelEstimator` with
    ...
    >>> PDSCH_transmitter = PDSCHTransmitter(PDSCH_config)
    >>> PDSCH_receiver = PDSCHReceiver(PDSCH_transmitter)
    >>> channel = AWGN()
    >>> x, b = PDSCH_transmitter(16)
    >>> no = 0.1
    >>> y = channel([x, no])
    >>> b_hat = PDSCH_receiver([x, no])
    >>> compute_ber(b, b_hat)
    ...
    def __init__(self,
        ...
        self._resource_grid = PDSCH_transmitter.resource_grid
        ...
        if self._input_domain=="time":
            ...
            self._ofdm_demodulator = OFDMDemodulator(
                fft_size=PDSCH_transmitter._num_subcarriers,
                l_min=self._l_min,
                cyclic_prefix_length=PDSCH_transmitter._cyclic_prefix_length)
        ...
        if channel_estimator is None:
            ...
            self._channel_estimator = sionna.nr.PDSCHLSChannelEstimator(
                                self.resource_grid,
                                PDSCH_transmitter._dmrs_length,
                                PDSCH_transmitter._dmrs_additional_position,
                                PDSCH_transmitter._num_cdm_groups_without_data,
                                interpolation_type='lin',
                                dtype=dtype)
        elif channel_estimator=="perfect":
            # Perfect channel estimation
            self._perfect_csi = True
            if PDSCH_transmitter._precoding=="codebook":
                self._w = PDSCH_transmitter._precoder._w
        ...
        if stream_management is None:
            ...
            rx_tx_association = np.ones([1, PDSCH_transmitter._num_tx], bool)
            self._stream_management = StreamManagement(
                                        rx_tx_association,
                                        PDSCH_transmitter._num_layers)
        else:
        ...
        if mimo_detector is None:
            ...
            self._mimo_detector = LinearDetector("lmmse", "bit", "maxlog",
                                        PDSCH_transmitter.resource_grid,
                                        self._stream_management,
                                        "qam",
                                        PDSCH_transmitter._num_bits_per_symbol,
                                        dtype=dtype)
        else:
        ...
        self._layer_demapper = sionna.nr.LayerDemapper(
                    PDSCH_transmitter._layer_mapper,
                    num_bits_per_symbol=PDSCH_transmitter._num_bits_per_symbol)

        # Use or create default TBDecoder
        if tb_decoder is None:
            # Default TBEncoder
            self._tb_decoder = sionna.nr.TBDecoder(
                                    PDSCH_transmitter._tb_encoder,
    ...
    def resource_grid(self):
        """OFDM resource grid underlying the PDSCH transmissions"""
