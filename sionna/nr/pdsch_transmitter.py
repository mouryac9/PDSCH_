...
#
"""PDSCH Transmitter for the nr (5G) sub-package of the Sionna library.
"""
...
class PDSCHTransmitter(Layer):
    # pylint: disable=line-too-long
    r"""PDSCHTransmitter(PDSCH_configs, return_bits=True, output_domain="freq", dtype=tf.complex64, verbose=False, **kwargs)

    This layer generates batches of 5G NR PDSCH slots for multiple transmitters
    ...
    .. figure:: ../figures/PDSCH_transmitter_block_diagram.png
        :scale: 30%
        :align: center
    ...
    If precoding is enabled in the :class:`~sionna.nr.PDSCHConfig`, the resource grids
    ...
    PDSCH_configs : instance or list of :class:`~sionna.nr.PDSCHConfig`
        PDSCH Configurations according to which the resource grid and pilot pattern
    ...
    def __init__(self,
        ...
        params = check_PDSCH_configs(PDSCH_configs)
        for key, value in params.items():
            self.__setattr__(f"_{key}", value)

        self._PDSCH_configs = PDSCH_configs
        ...
        self._tb_encoder = TBEncoder(
                            ...
                            n_rnti=self._n_rnti,
                            n_id=self._n_id,
                            channel_type="PDSCH", # PDSCHTransmitter
                            codeword_index=0, # not supported for PDSCH
                            use_scrambler=True,
                            verbose=self._verbose,
                            output_dtype=dtype.real_dtype)

        # Create PDSCHLayerMapper
        ...
        # Create PDSCHPilotPattern
        self._pilot_pattern = PDSCHPilotPattern(self._PDSCH_configs,
                                                dtype=dtype)
        ...
        # (Optionally) Create PDSCHPrecoder
        if self._precoding=="codebook":
            self._precoder = PDSCHPrecoder(self._precoding_matrices,
                                           dtype=dtype)
    ...
    def show(self):
        ...
        self._PDSCH_configs[0].carrier.show()
        Config.show(self._PDSCH_configs[0])
        for idx,p in enumerate(self._PDSCH_configs):
            print(f"---- UE {idx} ----")
            p.dmrs.show()
            p.tb.show()
    ...
    def call(self, inputs):
        ...
        # (Optionally) apply PDSCH precoding
        if self._precoding=="codebook":
